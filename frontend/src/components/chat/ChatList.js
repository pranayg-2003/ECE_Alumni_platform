...
const ChatList = ({ onSelectChat }) => {
  ...
  if (conversations.length === 0) {
    return (
      <div className="flex min-h-[240px] flex-col items-center justify-center rounded-[22px] border border-dashed border-black/[0.1] bg-white/80 px-6 py-12 text-center">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#1d1d1f] to-[#0071e3]/80 text-2xl shadow-md">
          💬
        </div>
      ...
      {conversations.map((conversation) => {
        ...
        return (
          <button
            ...
            className={`group w-full rounded-[20px] border text-left transition ${
              hasUnread
          ...
          >
            <div className="flex items-center gap-3 p-3.5 sm:p-4">
              <div className="relative shrink-0">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#1d1d1f] text-[15px] font-semibold text-white shadow-inner ring-2 ring-white">
                  {initial}
                </div>
                {isOnline && (
                  <span
